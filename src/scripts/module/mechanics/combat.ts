import { type NGHTrackedAttributeKey } from "./attribute-tracks.js";
import { type NGHParsedCard, parseCard } from "./card-usage.js";
import { type NGHSkillKey } from "./skill-tests.js";

export type NGHWeaponEffectKey =
  | "bonusDamage"
  | "stun"
  | "aim"
  | "overwhelm"
...
export interface NGHWeaponEffectDefinition {
  key: NGHWeaponEffectKey;
  cost: number;
  label: string;
}

export interface NGHWeaponProfile {
  id: string;
  label: string;
  skill: Extract<NGHSkillKey, "walkaWrecz" | "walkaDystansowa">;
  targetAttribute: NGHTrackedAttributeKey;
  baseDamage: number;
  effects: NGHWeaponEffectDefinition[];
}

export interface NGHWeaponAttackOptions {
  weaponId: string;
  successes: number;
  defense: number;
  selectedEffects?: NGHWeaponEffectKey[];
  rng?: () => number;
}

export interface NGHWeaponAttackResult {
  weapon: NGHWeaponProfile;
  successes: number;
  defense: number;
  hit: boolean;
  surplus: number;
  baseDamage: number;
  totalDamage: number;
  selectedEffects: NGHWeaponEffectDefinition[];
  availableEffects: NGHWeaponEffectDefinition[];
  ignoredArmorOnNextAttack: boolean;
  extraTargets: number;
  followUpAttack: boolean;
  targetArmorReducedBy: number;
  criticalDamage: number | null;
}

export interface NGHInitiativeEntry {
  id: string;
  label: string;
  card?: string | null;
...
export interface NGHResolvedInitiativeEntry extends NGHInitiativeEntry {
  parsedCard: NGHParsedCard | null;
  score: number;
  tied: boolean;
}
...
const WEAPON_LABEL_KEYS: Record<string, string> = {
...
const EFFECT_LABEL_KEYS: Record<NGHWeaponEffectKey, string> = {
...
const rollD6 = (rng: () => number = Math.random): number => Math.floor(rng() * 6) + 1;

export const NGH_WEAPON_PROFILES: Record<string, NGHWeaponProfile> = {
  fistsWeak: {
    id: "fistsWeak",
...
const uniqueEffects = (keys: NGHWeaponEffectKey[]): NGHWeaponEffectKey[] => [...new Set(keys)];

const localizeWeaponProfile = (profile: NGHWeaponProfile): NGHWeaponProfile => ({
  ...profile,
...
export const getWeaponProfile = (weaponId: string): NGHWeaponProfile => {
  const profile = NGH_WEAPON_PROFILES[weaponId];
  if (!profile) throw new Error(tf("NGH.Error.UnknownWeaponProfile", { weaponId }, `Unknown weapon profile: ${weaponId}`));
  return localizeWeaponProfile(profile);
};

export const resolveWeaponAttack = (options: NGHWeaponAttackOptions): NGHWeaponAttackResult => {
  ...
  let criticalDamage: number | null = null;
...
export const resolveInitiativeOrder = (entries: NGHInitiativeEntry[]): NGHResolvedInitiativeEntry[] => {
 ...
 */
export const computeAttackPT = (targetKrzepa: number, targetArmor: number): number => {
 ...
 */
export const resolveTieBreaker = (cardA: string | null | undefined, cardB: string | null | undefined): number => {
 ...
 */
export const getNextRoundStarterOptions = (
  allActorIds: string[],
  lastActorId: string
): string[] => {
