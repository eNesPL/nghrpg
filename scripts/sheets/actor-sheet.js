...
export class NGHActorSheet extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
    ...
    _onRender(_context, _options) {
        ...
        root.querySelectorAll("[data-action='advance-whisper']").forEach((btn) => {
            btn.addEventListener("click", (e) => {
                const whisper = e.currentTarget.dataset.whisper ?? "";
    ...
    async promptText(message, fallback = "") {
        ...
        const result = await foundry.applications.api.DialogV2.prompt({
            ...
            ok: {
                ...
                callback: (_event, _button, dialog) => {
                    ...
                    const input = dialogRoot?.querySelector("input[name='value']");
                    return input?.value ?? "";
                },
            },
            rejectClose: false,
    ...
    async promptPostRollModifiers(rolls, budgetRemaining) {
        if (rolls.length < 1) {
            await foundry.applications.api.DialogV2.prompt({
                window: { title: this.t("NGH.Dialog.PostRollTitle", "Post-roll modifiers") },
    ...
    async promptCardSelection(title, hint, hand) {
        if (hand.length === 0) {
            await foundry.applications.api.DialogV2.prompt({
                window: { title },
    ...
    async _doAdvanceAttribute(attributeKey) {
        ...
        if (!attr)
            throw new Error(this.t("NGH.Error.CardCodeRequired", "Unknown attribute"));
    ...
    async _doAdvanceSkill(skillKey) {
        ...
        if (!skill)
            throw new Error(this.t("NGH.Error.SkillKeyRequired", "Unknown skill"));
        if (!skill.trained) {
            const alreadyBought = await this.promptNumber(this.t("NGH.Prompt.NewSkillsAlreadyBought", "New skills bought this session (before this one)?"), 0);
            if (alreadyBought === null)
                return;
            const baseCost = 1 + (alreadyBought + 1);
            const hand = this.api.mechanics.cards.getHand(this.userId);
            const hint = this.tf("NGH.Advancement.NewSkillHint", { skill: skillKey, cost: baseCost }, `Acquire ${skillKey}. Cost: ${baseCost} cards. All faces/aces: starts at rank 1.`);
        ...
        const hand = this.api.mechanics.cards.getHand(this.userId);
        const hint = this.tf("NGH.Advancement.SkillDieHint", { skill: skillKey, current: currentRank, next: currentRank + 1, cost: baseCost }, `Advance ${skillKey}: rank ${currentRank} → ${currentRank + 1}. Cost: ${baseCost} cards.`);
    ...
    async _doAdvanceWhisper(whisperKey) {
        ...
        if (!whisper)
            throw new Error(this.t("NGH.Error.CardCodeRequired", "Unknown whisper path"));
        ...
        const hint = this.tf("NGH.Advancement.WhisperHint", { path: pathName, current: currentRank, next: currentRank + 1, cost: baseCost }, `Advance whisper ${pathName}: rank ${currentRank} → ${currentRank + 1}. Cost: ${baseCost} cards.`);
    ...
    async _doCastWhisper(whisperKey) {
        const system = this.actor.system;
        const whisper = system.whispers?.[whisperKey];
        if (!whisper)
            throw new Error(this.t("NGH.Error.CardCodeRequired", "Unknown whisper path"));
        ...
        const hand = this.api.mechanics.cards.getHand(this.userId);
        const spellName = await this.promptText(this.tf("NGH.Whisper.Cast.SpellPrompt", { path: pathName }, `Spell name for path "${pathName}" (leave blank to skip narrative step):`), "");
        if (spellName === null)
            return;
        const costCards = await this.promptCardSelection(this.t("NGH.Whisper.Cast.CostTitle", "Pay Whisper Cost"), this.tf("NGH.Whisper.Cast.CostHint", { path: pathName }, `Select 1 card from hand to burn as whisper casting cost for path "${pathName}".`), hand);
        if (!costCards || costCards.length < 1)
            return;
    ...
    async _doCorruptionRiskCheck(reason) {
        ...
        if (riskResult.isBlack) {
            ...
            await this.actor.update({ "system.corruption": Math.min(5, current + 1) });
        }
        const lines = [
            this.tf("NGH.Chat.CorruptionRisk.Reason", { reason }, `Reason: ${reason}`),
            ...riskResult.jokersSkipped.map((joker) => this.tf("NGH.Chat.CorruptionRisk.JokerSkipped", { joker }, `Joker skipped: ${joker}`)),
    ...
    async _doElementalRitual() {
        ...
        let selectedCard;
        if (hand.length > 0) {
            const selected = await this.promptCardSelection(this.t("NGH.Action.ElementalRitual", "Elemental Ritual"), this.t("NGH.ElementalRitual.SelectCostCard", "Select a card to pay Elemental Ritual cost."), hand);
        ...
        const lines = [
            result.card
                ? this.tf(result.source === "top-deck" ? "NGH.Chat.ElementalRitual.Drew" : "NGH.Chat.ElementalRitual.Paid", { card: result.card }, result.source === "top-deck" ? `Drew from deck: ${result.card}` : `Discarded from hand: ${result.card}`)
                : this.t("NGH.Chat.ElementalRitual.NoDraw", "No card available in deck."),
        ];
        if (result.card) {
            lines.push(result.source === "top-deck"
                ? (result.isBlack
    ...
    async _doOfficerShareCards() {
        const players = Array.from((game.users?.contents ?? []))
            .filter((u) => u?.id && !u.isGM && u.active)
        ...
        const confirmed = await this.promptConfirm(this.t("NGH.Officer.DialogHint", "You will risk Corruption, then draw 4 cards from the deck and assign each to a player."));
        if (!confirmed)
            return;
        await this._doCorruptionRiskCheck(this.t("NGH.CorruptionRisk.Reason.OfficerShare", "Officer — sharing cards with squad"));
        const selections = [];
        for (let i = 1; i <= 4; i += 1) {
            const value = await this.promptText(this.tf("NGH.Officer.CardPrompt", { n: i, players: playerList }, `Card ${i} recipient (${playerList})`), players[(i - 1) % players.length]?.name ?? "");
            if (value === null)
                return;
    ...
    async _doGMCorruptionIncident() {
        ...
        const detail = await this.promptText(this.t("NGH.Prompt.CorruptionIncident", "Corruption incident reason (optional)"), "");
        if (detail === null)
            return;
    ...
    async _doDiscardCard(card) {
        if (!card)
            throw new Error(this.t("NGH.Error.CardCodeRequired", "Card action requires a card code"));
    ...
    async _doBurnCard(card, reason) {
        ...
        await this.createChatMessage(reason === "plus-one"
    ...
    async _doHealCard(card) {
        ...
        const result = await this.actor.healAttributeDamage(attribute, 1);
    ...
    async _doDefendCard(card) {
        ...
        if (incomingDamage === null)
            return;
        const result = await this.actor.preventDamageWithCards(attribute, incomingDamage, [card], this.userId);
    ...
    async _doSkillTest(skill) {
        if (!skill)
            throw new Error(this.t("NGH.Error.SkillKeyRequired", "Skill test action requires a skill key"));
        const system = this.actor.system;
    ...
    async _doChallengeTest(skill) {
        ...
        const participantsRaw = await this.promptText(this.tf("NGH.Prompt.ChallengeParticipants", { available }, `Challenge participants (comma-separated actor IDs or names). Available: ${available}`), String(this.actor.name ?? this.actor.id));
        if (!participantsRaw)
            return;
        const tokens = participantsRaw.split(",").map((value) => value.trim()).filter(Boolean);
        const selectedActors = tokens
            .map((token) => actors.find((actor) => String(actor.id) === token) ??
            actors.find((actor) => String(actor.name ?? "").toLocaleLowerCase() === token.toLocaleLowerCase()))
            .filter((actor, index, list) => Boolean(actor) && list.indexOf(actor) === index);
        ...
        await this.createChatMessage(this.t("NGH.Chat.Challenge.Title", "Group Challenge"), [
            ...
            result.passed ? this.t("NGH.Chat.Result.Success", "Result: success") : this.t("NGH.Chat.Result.Failure", "Result: failure"),
            ...result.participants.map((entry) => this.tf("NGH.Chat.Challenge.Participant", { participant: entry.label, successes: entry.successes, rolls: entry.rolls.join(", ") }, `${entry.label}: ${entry.successes} success(es) [${entry.rolls.join(", ")}]`))
        ]);
    }
    async _doWeaponAttack() {
        ...
        if (preview.availableEffects.length > 0) {
            const effectInput = await this.promptText(this.tf("NGH.Prompt.AvailableEffects", { effects: preview.availableEffects.map((effect) => `${effect.key} (${effect.cost})`).join(", ") }, `Available effects: ${preview.availableEffects.map((effect) => `${effect.key} (${effect.cost})`).join(", ")}\nType comma-separated effect keys to apply, or leave blank.`), "");
        ...
        await this.createChatMessage(this.t("NGH.Chat.WeaponAttack.Title", "Weapon Attack"), [
            ...
            attackResult.selectedEffects.length > 0
                ? this.tf("NGH.Chat.WeaponAttack.Effects", { effects: attackResult.selectedEffects.map((effect) => effect.label).join(", ") }, `Effects: ${attackResult.selectedEffects.map((effect) => effect.label).join(", ")}`)
