import { resolveInitiativeOrder, getNextRoundStarterOptions, resolveTieBreaker, } from "../module/mechanics/combat.js";
...
const parseEnemyGroupLine = (line) => {
...
export const registerCombatPanelSettings = () => {
    game.settings?.register(NGH_SYSTEM_ID, COMBAT_STATE_SETTING, {
        name: "Combat State",
        scope: "world",
        config: false,
        type: String,
...
const getCombatState = () => {
    try {
        return JSON.parse(game.settings?.get(NGH_SYSTEM_ID, COMBAT_STATE_SETTING));
    }
    catch {
        return { ...DEFAULT_STATE, entries: [] };
    }
};
const setCombatState = async (state) => {
    await game.settings?.set(NGH_SYSTEM_ID, COMBAT_STATE_SETTING, JSON.stringify(state));
};
let panelInstance = null;
export const openCombatPanel = () => {
    if (!game.user?.isGM) {
        ui.notifications?.warn(game.i18n?.localize("NGH.Error.GMOnlyCombatPanel") ??
...
export class NGHCombatPanel extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    t(key, fallback) {
    ...
    async _prepareContext() {
        ...
        const isLastToAct = unacted.length === 0 ||
        ...
        const entriesWithFlags = entries.map((e) => ({
            ...
            kindLabel: e.kind === "enemy-group"
                ? this.t("NGH.Combat.Kind.EnemyGroup", "Enemy group")
                : this.t("NGH.Combat.Kind.Character", "Character"),
            groupInfo: e.kind === "enemy-group"
                ? this.tf("NGH.Combat.GroupInfo", { size: e.groupSize, bonus: e.groupAttackBonus }, `size ${e.groupSize}, atk ${e.groupAttackBonus >= 0 ? `+${e.groupAttackBonus}` : e.groupAttackBonus}`)
                : ""
        }));
        ...
        return {
            ...
            canStartCombat: phase === "initiative" &&
                entries.length > 0 &&
    ...
    async _doNewCombat() {
        ...
        const enemyInput = (await foundry.applications.api.DialogV2.prompt({
            ...
            content: `<form class="ngh-dialog"><p>${this.t("NGH.Combat.EnemyGroups.Prompt", "Enter enemy groups, one per line:")}</p><textarea name="enemies" style="width:100%;height:80px;"></textarea></form>`,
            ok: {
                label: this.t("NGH.Action.Confirm", "OK"),
                callback: (_event, _button, dialog) => {
                    const root = this.getDialogRoot(dialog);
                    return (root?.querySelector("textarea[name='enemies']")
                        ?.value ?? "");
                },
            },
            rejectClose: false,
        }));
        const enemyEntries = (enemyInput ?? "")
            .split("\n")
            .map((line) => line.trim())
    ...
    async _doResolveOrder() {
        const state = getCombatState();
        const root = this.element;
        const updatedEntries = state.entries.map((entry) => {
            const cardInput = root.querySelector(`input[name="card-${entry.id}"]`);
            const tbInput = root.querySelector(`input[name="tb-${entry.id}"]`);
            return {
                ...entry,
        ...
        while (swapped) {
            ...
            for (let i = 0; i < newEntries.length - 1; i++) {
                if (newEntries[i].tied &&
                    newEntries[i + 1].tied &&
                    (newEntries[i].tieBreakerCard || newEntries[i + 1].tieBreakerCard)) {
                    const tbScore = resolveTieBreaker(newEntries[i].tieBreakerCard, newEntries[i + 1].tieBreakerCard);
                    if (tbScore < 0) {
    ...
    async _doStartCombat() {
        ...
        const options = state.entries
            .map((e) => `<option value="${e.id}">${e.label} (${this.t("NGH.Combat.Score", "Score")}: ${e.score})</option>`)
            .join("");
        const firstActorId = (await foundry.applications.api.DialogV2.prompt({
            ...
            content: `<form class="ngh-dialog"><p>${this.t("NGH.Combat.ChooseFirst.Prompt", "The actor with highest initiative chooses who acts first:")}</p><select name="first" style="width:100%">${options}</select></form>`,
            ok: {
                label: this.t("NGH.Action.Confirm", "OK"),
                callback: (_event, _button, dialog) => {
                    const root = this.getDialogRoot(dialog);
                    return (root?.querySelector("select[name='first']")
                        ?.value ?? "");
                },
            },
            rejectClose: false,
        ...
        const firstLabel = state.entries.find((e) => e.id === firstActorId)?.label ?? firstActorId;
        await this.createChatMessage(this.t("NGH.Chat.Combat.StartTitle", "Combat Started"), [
            this.tf("NGH.Chat.Combat.Round", { round: 1 }, "Round 1"),
            this.tf("NGH.Chat.Combat.FirstActor", { actor: firstLabel }, `First actor: ${firstLabel}`),
        ]);
        void this.render(true);
    }
    async _doPassTurn() {
        ...
        const newEntries = entries.map((e) => e.id === activeActorId ? { ...e, acted: true } : e);
        ...
        const nextActorId = (await foundry.applications.api.DialogV2.prompt({
            ...
            content: `<form class="ngh-dialog"><p>${this.t("NGH.Combat.ChooseNext.Prompt", "Choose who acts next:")}</p><select name="next" style="width:100%">${options}</select></form>`,
            ok: {
                label: this.t("NGH.Action.Confirm", "OK"),
                callback: (_event, _button, dialog) => {
                    const root = this.getDialogRoot(dialog);
                    return (root?.querySelector("select[name='next']")
                        ?.value ?? "");
                },
            },
            rejectClose: false,
        ...
        if (!nextActorId)
            return;
        const nextLabel = remaining.find((e) => e.id === nextActorId)?.label ?? nextActorId;
        await this.createChatMessage(this.t("NGH.Chat.Combat.PassTitle", "Initiative Passed"), [
            this.tf("NGH.Chat.Combat.NextActor", { actor: nextLabel }, `Next: ${nextLabel}`),
        ]);
        await setCombatState({
            ...state,
            entries: newEntries,
    ...
    async _doEndRound() {
        const state = getCombatState();
        const { entries, activeActorId } = state;
        const allActed = entries.map((e) => e.id === activeActorId ? { ...e, acted: true } : e);
        const lastActorId = activeActorId ?? "";
        const eligibleIds = getNextRoundStarterOptions(entries.map((e) => e.id), lastActorId);
        const eligible = eligibleIds
            .map((id) => entries.find((e) => e.id === id))
            .filter((e) => Boolean(e));
        if (eligible.length === 0) {
            ui.notifications?.warn(this.t("NGH.Combat.NoEligibleStarter", "No eligible starter for the next round."));
            return;
        }
        ...
        const starterActorId = (await foundry.applications.api.DialogV2.prompt({
            window: {
                title: this.t("NGH.Combat.ChooseStarter.Title", "Next Round Starter"),
            },
            content: `<form class="ngh-dialog"><p>${this.t("NGH.Combat.ChooseStarter.Prompt", "The last actor chooses who starts the next round (not themselves):")}</p><select name="starter" style="width:100%">${options}</select></form>`,
            ok: {
                label: this.t("NGH.Action.Confirm", "OK"),
                callback: (_event, _button, dialog) => {
                    const root = this.getDialogRoot(dialog);
                    return (root?.querySelector("select[name='starter']")
                        ?.value ?? "");
                },
            },
            rejectClose: false,
        ...
        const starterLabel = eligible.find((e) => e.id === starterActorId)?.label ?? starterActorId;
        await setCombatState({
            ...state,
            round: newRound,
        ...
        await this.createChatMessage(this.t("NGH.Chat.Combat.NewRoundTitle", "New Round"), [
            this.tf("NGH.Chat.Combat.Round", { round: newRound }, `Round ${newRound}`),
            this.tf("NGH.Chat.Combat.FirstActor", { actor: starterLabel }, `First actor: ${starterLabel}`),
        ]);
        void this.render(true);
    }
    async _doReset() {
    ...
    async _doEnemyGroupAttack() {
        ...
        await this.createChatMessage(this.t("NGH.Chat.Combat.GroupAttack.Title", "Enemy Group Attack"), [
