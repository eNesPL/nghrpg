...
export const openGMToolsPanel = () => {
    if (!game.user?.isGM) {
        ui.notifications?.warn(game.i18n?.localize("NGH.Error.GMOnlyGMToolsPanel") ??
...
export class NGHGMToolsPanel extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    t(key, fallback) {
    ...
    readJsonArraySetting(setting) {
        const raw = String(game.settings?.get(NGH_SYSTEM_ID, setting) ?? "[]");
        try {
    ...
    getCorruptionJourneyCards(corruption) {
        if (corruption >= 4)
            return 2;
        if (corruption >= 3)
            return 1;
    ...
    getCharacterViews() {
        ...
        const usersByActorId = new Map();
        for (const user of users) {
            const actorId = String(user?.character?.id ?? "");
            if (actorId)
                usersByActorId.set(actorId, user);
        }
    ...
    getJourneyStateSummary() {
        const state = this.api?.mechanics?.cards?.getState?.() ?? { hands: {}, journeyHands: {}, drawPile: [], discardPile: [] };
        const poolCards = this.parseCards(this.readJsonArraySetting(JOURNEY_POOL_SETTING));
        const journeyHands = Object.values((state.journeyHands ?? {}));
        const standardHands = Object.values((state.hands ?? {}));
        return {
            phase: this.t(`NGH.Journey.Phase.${String(game.settings?.get(NGH_SYSTEM_ID, JOURNEY_PHASE_SETTING) ?? "configure")}`, String(game.settings?.get(NGH_SYSTEM_ID, JOURNEY_PHASE_SETTING) ?? "configure")),
            ...
            poolCards,
            extraBlindCount: Math.max(0, Number(game.settings?.get(NGH_SYSTEM_ID, JOURNEY_EXTRA_BLIND_COUNT_SETTING) ?? 0)),
    ...
    async _prepareContext() {
        ...
        return {
            ...
            subtitle: this.t("NGH.GMTools.Subtitle", "Session-facing quick references and checklists for Never Going Home."),
    ...
    _onRender(_context, _options) {
        ...
        const search = root.querySelector("[data-gm-screen-search]");
        const cards = Array.from(root.querySelectorAll("[data-gm-rule-card]"));
        const sections = Array.from(root.querySelectorAll("[data-gm-rule-section]"));
        const empty = root.querySelector("[data-gm-screen-empty]");
        const count = root.querySelector("[data-gm-screen-count]");
        ...
        const applyFilter = () => {
            ...
            for (const section of sections) {
                const hasVisibleCard = Array.from(section.querySelectorAll("[data-gm-rule-card]"))
