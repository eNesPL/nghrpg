...
export class NGHJourneyPanel extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    _onRender(_context, _options) {
        ...
        if (requirementsField) {
            requirementsField.addEventListener("blur", () => {
                void game.settings?.set(NGH_SYSTEM_ID, JOURNEY_REQUIREMENTS_SETTING, requirementsField.value ?? "");
        ...
        if (bonusField) {
            bonusField.addEventListener("blur", () => {
                void game.settings?.set(NGH_SYSTEM_ID, JOURNEY_BONUS_SETTING, bonusField.value ?? "");
                broadcastJourneyUpdate();
            });
        }
        ...
        if (penaltyField) {
            penaltyField.addEventListener("blur", () => {
                void game.settings?.set(NGH_SYSTEM_ID, JOURNEY_PENALTY_SETTING, penaltyField.value ?? "");
                broadcastJourneyUpdate();
            });
        }
    }
    ...
    async _doRevealSlot(slotIndex) {
        ...
        if (slot.card === BLACK_JOKER) {
            ...
            if (isGMSlot) {
                await this.createChatMessage(this.t("NGH.Chat.Journey.BJGMSlotTitle", "Black Joker — Blind Slot"), [this.t("NGH.Chat.Journey.BJGMSlotWarning", "A Black Joker appeared in a blind/surplus journey slot. Assign corruption risk to the appropriate player manually.")]);
            }
            else {
                ...
                await this.createChatMessage(this.t("NGH.Chat.CorruptionRisk.Title", "Corruption Risk"), riskLines);
            }
        }
    }
    get api() {
    ...
    async _doRevealAllCards() {
        ...
        await this.setRevealedCards(revealed);
        await this.createChatMessage(this.t("NGH.Chat.Journey.RevealTitle", "Journey Reveal"), slots.map((slot) => this.tf("NGH.Chat.Journey.RevealedCard", { user: slot.userName, card: slot.card }, `${slot.userName}: ${slot.card}`)));
        void this.render();
        broadcastJourneyUpdate("reveal");
